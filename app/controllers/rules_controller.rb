class RulesController < ApplicationController
  ...
  def search
    @results = Transaction.where("category LIKE ?", "%#{params[:keywords]}%")
    respond_to do |format|
      format.html
      format.turbo_stream
      # format.turbo_stream { render partial: "rules/transaction", collection: @results }
  ...
  private
    ...
    def rule_params
      params.require(:rule).permit(:name, :keyword)
    end
end
